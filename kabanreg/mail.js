    ...
    getVerLink: function(str,csrf) {
        return new Promise((resolve, reject) => {
            mail.getMessage(str, csrf, function(e, b) {
                ...
                resolve({
                    success: true,
                    message: `https://click.discord.com/` + b.split(`https://click.discord.com/`)[2].split("\"")[0]
                });
            })
        })
    },
                    ...
                    return connection.search(searchCriteria, fetchOptions).then(function (messages) {
                        messages.forEach(function (item) {
                            ...
                            simpleParser(idHeader+all.body, (err, mail) => {
                                if(mail.subject.search(`Discord`) > -1){
                                    const krut = mail.html.split(`Подтвердить e-mail`)[0].slice(-2000).match(/http[^ ]+/g)[0].slice(0,-1);
                                    resolve(krut)
                                }
                            });
                        });
